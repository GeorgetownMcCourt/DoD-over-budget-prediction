...
pred.check <- function(x,y) { 
  ...
  print(meanf1(x$IsTerminated, pred)) 
}
...
df$UnmodifiedIsFullAndOpen[is.na(df$UnmodifiedIsFullAndOpen)] <- 3
...
{
set.seed(100)
sample_train <- floor(.7*nrow(df))
sample_valid <- floor(.15*nrow(df))

index_train <- sort(sample(seq_len(nrow(df)), size=sample_train))
index_not.train <- setdiff(seq_len(nrow(df)), index_train)
...
#smaller dataset because not enough memory. Use smaller set to find relevant variables
df_sub <- df[sample(nrow(df), 100000),]

###random forest classification
set.seed(123)
rf <- randomForest(IsTerminated ~ UnmodifiedIsInternational + 

...
set.seed(321)
df_sub <- df_comp[sample(nrow(df_comp), 1000000),]
...
{
  set.seed(100)
  sample_train <- floor(.7*nrow(df_sub))
  sample_valid <- floor(.15*nrow(df_sub))
  
  index_train <- sort(sample(seq_len(nrow(df_sub)), size=sample_train))
  index_not.train <- setdiff(seq_len(nrow(df_sub)), index_train)
  index_valid  <- sort(sample(index_not.train, size=sample_valid))
  index_test  <- setdiff(index_not.train, index_valid)
  
  df_train  <- df_sub[index_train, ]
  df_valid <- df_sub[index_valid, ]
  df_test <- df_sub[index_test, ]
}
...
tuneRF(df_sub[,
                ...
                c("UnmodifiedContractObligatedAmount" , 
                    "UnmodifiedContractBaseAndExercisedOptionsValue" , 
                    "UnmodifiedContractBaseAndAllOptionsValue" ,
                    "UnmodifiedCurrentCompletionDate" ,
                    "UnmodifiedDays" ,

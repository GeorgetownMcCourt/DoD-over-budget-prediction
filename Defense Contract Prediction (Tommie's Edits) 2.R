...
nrow(full.data[full.data$Term == "Terminated" & full.data$IsClosed == "Closed",]) # Al 853 terminated were marked as closed (phew)
...
#remove all NAs
complete.data <- full.data[complete.cases(full.data),] #~300 cases lost
...
full.data$CSIScontractID <- as.factor(full.data$CSIScontractID) #why do this?

#prelimnary linear model to see general contributory impact of other variables
# Note that at the moment
full.data$Term.2 <- ifelse(full.data$Term == "Terminated", c(1), c(0))

summary(lm(Term.2 ~ Who + What + Intl + PSR + FxCb + Fee + Dur + SingleOffer + Soft +
             UCA + CRai + Veh + UnmodifiedNumberOfOffersReceived, data = full.data))
#logit model
summary(glm(Term ~ Who + What + Intl + PSR + FxCb + Fee + Dur + SingleOffer + Soft +
              UCA + CRai + Veh + UnmodifiedNumberOfOffersReceived, family = "binomial"(link = logit), data = full.data))

####
#### Model Training Section ####
####

#Train, Test, Validate partition
set.seed(100)
sample_train <- floor(.7*nrow(complete.data))
sample_valid <- floor(.15*nrow(complete.data))

index_train <- sort(sample(seq_len(nrow(complete.data)), size=sample_train))
index_not.train <- setdiff(seq_len(nrow(complete.data)), index_train)
index_valid  <- sort(sample(index_not.train, size=sample_valid))
index_test  <- setdiff(index_not.train, index_valid)

complete_train  <- complete.data[index_train, ]
complete_valid <- complete.data[index_valid, ]
complete_test <- complete.data[index_test, ]
...
complete.data$folds <- kfolds.index(nrow(complete.data), j)

#Run kfolds, cycling through each k has the holdout
set.seed(548)
for(k in unique(complete.data$folds)){
  
  #Cut train/test: k is the holdhout
  this.train <- complete.data[complete.data$folds != k, ]
  this.test <- complete.data[complete.data$folds == k, ]
  ...
  this.rf <- randomForest(Term ~  Who + What + Intl + PSR + FxCb + Fee + Dur + SingleOffer + Soft +
                          ...
                          mtry = 3, ntree = 200, na.action = na.omit, 
  ...
  this.pred <- pred.check(this.test, this.rf)
  
  #Print the meanf1 accuracy measure for this iteration of the model
  print(paste0("meanF1 (holdout group ", k,") = ", this.pred))
...
this.rf <- randomForest(Term ~  Who + What + Intl + PSR + FxCb + Fee + Dur + Soft +
                          UCA + CRai + Veh + UnmodifiedNumberOfOffersReceived, data = complete_train, #note using train data
                        mtry = 4, ntree = 200, 
                        #omit missing values - or should we impute (using rfImpute)?
                        na.action = na.omit,
                        #importance = TRUE, #uncomment this line to determine variable importance
                        type = "classification")

#determine which variables to remove (i.e. has too little influence)
varImpPlot(this.rf)

#find optimal mtry (should be 4)
tuneRF(complete.data[,c("Who", "What","Intl", "PSR", "FxCb", "Fee", "Dur", 
                        "Soft", "UCA", "CRai", "Veh", "UnmodifiedNumberOfOffersReceived")], complete.data$Term, ntreeTry = 400, na.action = na.omit, 
       mtryStart = 1, stepFactor = 2, 
       improve = 0.001, trace = TRUE, plot = TRUE)

#checking predition
pred.check(complete_train, this.rf)
pred.check(complete_valid, this.rf)
pred.check(complete_test, this.rf)
